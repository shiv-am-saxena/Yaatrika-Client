...
export function Signup() {
    ...
    const verifyOtp = async (e) => {
        ...
        try {
            dispatch(setLoading(true));
            const response = await axiosInstance.post("/auth/user/verify-otp", { phoneNumber: phone, otp });
            const res = response.data;
            if (res.success) {
                showSuccessToast(res.message);
                setIsVerified(true);
    ...
    const registerHandler = async (e) => {
        ...
        if (!firstName || !lastName || !email || !gender || !countryCode || !phone || !password || !cnfPassword) {
            showErrorToast("All fields are required");
            return;
        }

        if (!agreement) {
            showErrorToast("Agree to Terms & Conditions.");
            return;
        }
            ...
            <div
                ...
                <form className="my-8" onSubmit={registerHandler}>
                    ...
                    <LabelInputContainer className="mb-4">
                        ...
                        <Input id="password" placeholder="••••••••" type={type ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} />
                    </LabelInputContainer>
                    ...
                    <LabelInputContainer className={"mb-0 flex flex-row gap-2 p-1"}>
                        <Checkbox id="show" onClick={() => setType(!type)} checked={type} />
                        <Label htmlFor="show" className="text-nowrap w-full">Show Password</Label>
                    </LabelInputContainer>
                    <LabelInputContainer className={"mb-6 flex flex-row gap-2 p-1"}>
                        <Checkbox id="agree" onClick={() => setAgreement(!agreement)} checked={agreement} />
                        <Label htmlFor="agree" className="text-nowrap w-full">Agree to all <Link to={'/terms-and-conditions'} className="text-blue-500">Terms & Conditions.</Link></Label>
                    </LabelInputContainer>
